...
`geom_quantile( mapping=None, data=None, *, stat="quantile", position="identity", na_rm=False, inherit_aes=True, show_legend=None, raster=False, lineend="butt", linejoin="round", **kwargs )`
...
| `stat` | str \| stat | `"quantile"` | The statistical transformation to use on the data for this layer. If it is a string, it must be the registered and known to Plotnine. |
...
| `linejoin` | Literal[round, miter, bevel] | `"round"` | Line join style. This option is applied for solid linetypes. |
