...
`geom_pointdensity( mapping=None, data=None, *, stat="pointdensity", position="identity", na_rm=False, inherit_aes=True, show_legend=None, raster=False, **kwargs )`
...
| `stat` | str \| stat | `"pointdensity"` | The statistical transformation to use on the data for this layer. If it is a string, it must be the registered and known to Plotnine. |
