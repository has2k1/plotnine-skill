...
`scale_alpha_datetime( date_breaks=None, date_labels=None, date_minor_breaks=None, range=(0.1, 1), *, name=None, breaks=True, limits=None, labels=True, expand=None, guide="legend", na_value=np.nan, aesthetics=(), rescaler=rescale, oob=censor, minor_breaks=True, trans="datetime" )`
...
| `range` | tuple[float, float] | `(0.1, 1)` | Range ([Minimum, Maximum]) of output alpha values. Should be between 0 and 1. |
