...
`geom_bar( mapping=None, data=None, *, stat="count", position="stack", na_rm=False, inherit_aes=True, show_legend=None, raster=False, just=0.5, width=None, **kwargs )`
...
| `stat` | str \| stat | `"count"` | The statistical transformation to use on the data for this layer. If it is a string, it must be the registered and known to Plotnine. |
